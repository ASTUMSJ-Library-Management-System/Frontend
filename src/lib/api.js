...
export const borrowAPI = {
  ...
  markAsReturned: async (borrowId) => {
    try {
      ...
      console.error('Mark As Returned API Error:', error.response?.status, error.response?.data);
      if (error.response?.status === 404) {
        throw new Error('Mark as returned endpoint not found. Please check your backend route.');
      }
      if (error.response?.data?.message) {
        throw new Error(error.response.data.message);
      } else {
